...
def score_technical(latest: pd.Series, sector_rsi: float, checks: dict) -> tuple:
    """
    Score a stock's latest row against the enabled technical checks.

    checks: dict of booleans - use_ema, use_roc, use_sector_rsi, use_vol
    ...
    if checks.get("use_sector_rsi"):
        total += 1
        if latest["RSI"] > sector_rsi:
            passed += 1
